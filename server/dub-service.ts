...
class DubService {
  ...
  async createReportLink(
    ...
    const slug = `czb-${reportId.slice(0, 8)}`;
    return this.createLink({
      url: reportUrl,
      customSlug: slug,
      externalId: `ext_report_${reportId}`,
      metadata: {
        title: `${clientName} - Cognitive Zero-Base Analysis`,
        description: "AI-powered business transformation analysis by BlueAlly"
      }
    });
  }
